...
async def next_page(bot, query):
    ...
    if int(ad_user) in ADMINS:
        ...
        return await query.answer(
            "⚠️ Bro, search your on file, don't click others request file🥵  ⚠️Bro മറ്റുള്ളവർ റിക്വസ്റ്റ് ചെയ്ത മൂവിയിൽ കുത്തി നോക്കാതെ ഡ്രോയിങ് വേണ്ടത് ബ്രോ റിക്വസ്റ്റ് ചെയ്യുക🤒",
            show_alert=True)
    try:
        offset = int(offset)
    except:
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb:
        ...
        cap = f"<b>🎬 Title:</b> {search}\n</b>\n<b><✍️ Note:</b> <s>This message will be Auto-deleted after 1 minute to avoid copyright issues.</s>"
